...
// Cloudflare Workers compatible config will be passed in via the app creation
import { logger } from '../core/logger.js';
import { CloudflareOpenAIClient as OpenAIClient } from '../core/client-cf.js';
...
} from '../conversion/response-converter-cf.js';

const app = new Hono();

// Cloudflare Workers compatible app will receive config via env
let config: any;
let openaiClient: OpenAIClient;

// Initialize config and client in middleware
app.use('*', async (c, next) => {
  // @ts-ignore
  config = c.env?.CONFIG || globalThis.CONFIG || configManager.config;
...
const validateApiKey = async (_c: any, next: any) => {
